document.addEventListener('DOMContentLoaded', function(){
    ...
    *   PAGE LOGIN & REGISTER
    */
    let pageLogin = document.querySelector('.app .wrap-login')

    if(pageLogin != null) {

        /*
        * POPUP VALIDATE PHONE OTP
        */

        /* STEP 1 - NHAP SO DIEN THOAI */
        let inputPhone = pageLogin.querySelector('.login-phone .txt_phone')
        let btnSubmitPhone = pageLogin.querySelector('.login-phone .submit-phone')
        let openFormOTP =  pageLogin.querySelector('.wrap-login-phone input[name="subLoginPhone"][id="open-otp"]')

        function setMessErrorInput(wrapElement, mess){
            wrapElement.parentNode.querySelector('.show-mess-error').innerHTML = mess
        }

        function validateNumberPhone(){
            let txtPhone = inputPhone.value
            let phone_regex = /((09|03|07|08|05)+([0-9]{8})\b)/g
            if(phone_regex.test(txtPhone) == false) {
                inputPhone.classList.add('error')
                btnSubmitPhone.disabled = true
                if(txtPhone == ''){
                    setMessErrorInput(inputPhone, 'Vui lòng nhập số điện thoại !')
                }else{
                    setMessErrorInput(inputPhone, 'Sai định dạng số điện thoại !')
                }
                return false
            }else{
        ...
        function checkInputNumberPhone(e){
            e.preventDefault();
            if(validateNumberPhone() == true) {
                openFormOTP.checked = true
            }
        }

        inputPhone.addEventListener('keyup', validateNumberPhone )
        btnSubmitPhone.addEventListener('click', checkInputNumberPhone )

        /* STEP 2 - NHAP OTP */
        let inputOTP = pageLogin.querySelector('.login-phone.otp .txt_otp')
        let btnSubmitOTP = pageLogin.querySelector('.login-phone.otp .submit-otp')
        let openFormDone =  pageLogin.querySelector('.wrap-login-phone input[name="subLoginPhone"][id="open-done"]')

        function validateOTP(e) {
            let txtOTP = inputOTP.value
            if(txtOTP.length == 6) {
                inputOTP.classList.remove('error')
            }
            e.preventDefault()
            inputOTP.scrollLeft = 0
            if(txtOTP.length > 6) {
                inputOTP.value = txtOTP.slice(0,6)
            }
        }

        function checkInputOTP(e){
            e.preventDefault();
            let txtOTP = inputOTP.value
            if(txtOTP.length == 6 ) {
                openFormDone.checked = true
            }else{
                inputOTP.classList.add('error')
                if(txtOTP.length == 0){
                    setMessErrorInput(inputOTP, 'Vui lòng nhập mã xác nhận!')
                }else{
                    setMessErrorInput(inputOTP, 'Vui lòng điền đầy đủ mã xác nhận!')
                }
            }
        }

        inputOTP.addEventListener('keyup', validateOTP )
        btnSubmitOTP.addEventListener('click', checkInputOTP )

        /* STEP 3 - OPEN POPUP REGISTER */
        let btnSubmitDone = pageLogin.querySelector('.login-phone.done .submit-done')
        let popupRegister = pageLogin.querySelector('.wrap-popup-register')

        btnSubmitDone.addEventListener('click', function(){
       ...
       let userRegister = {}
       let wrapFormValidate = pageLogin.querySelectorAll('.wrap-popup-register .wrap-popup-item')
       let listFormValidate = pageLogin.querySelectorAll('.wrap-popup-register .form-validate')

        /* render province address user step - 4 */
        ...
        function renderProvince(txt){
            let wrapProvince =  pageLogin.querySelector('.wrap-province')
            
            if(txt==null){
                for( let index in provinces) {
                    wrapProvince.innerHTML += ` <li class="province-item" data-codeProvince=${provinces[index].code}>
                        ${provinces[index].name}
                    </li>` 
                }
            }else{
                let txtValue;
                let listItemProvince = pageLogin.querySelectorAll('li.province-item')
                for (let i = 0; i < listItemProvince.length; i++) {
                    txtValue = listItemProvince[i].textContent || listItemProvince[i].innerText;
                    if (txtValue.toUpperCase().indexOf(txt) > -1) {
                        listItemProvince[i].style.display = "";
                    } else {
                        listItemProvince[i].style.display = "none";
                    }
                }
            }
        }

         /* render district address user step - 4 */
        function rederDistrictInProvince(code){
            let districts = {
                "100": {
                "name": "Điện Biên",
                "type": "huyen",
            ...
            for( let index in districts) {
                if(districts[index].parent_code  == code) {
                    districtsInProvince.push(districts[index])
                }
            }

            let wrapDistrict = pageLogin.querySelector('.wrap-district')
            wrapDistrict.innerHTML=`<option value="-1" selected >Vui lòng chọn quận/huyện</option>`
            districtsInProvince.forEach( item => {
                wrapDistrict.innerHTML += `<option value="${item.code}" >${ item.name }</option>`
            })
        }

        /** filter province key up step - 4  */
        function eventChooseAddressUser(){
            renderProvince()
            let formValidate = wrapFormValidate[4].querySelector('.form-validate')
            let input_province = formValidate.querySelector('.txt_province')
            let wrap_province = formValidate.querySelector('.wrap-province')
            let province_item = wrap_province.querySelectorAll('.province-item')
            
            input_province.addEventListener('focus', function(e){
                input_province.classList.remove('error')
                wrap_province.classList.add('active')
            })

            input_province.addEventListener('keyup', function (){
                renderProvince(this.value.toUpperCase())
            })

            province_item.forEach( province => {
                province.addEventListener('click', function () {
                    input_province.value = this.innerHTML.trim()
                    input_province.dataset.code = this.dataset.codeprovince
                    rederDistrictInProvince(this.dataset.codeprovince)
                    wrap_province.classList.remove('active')
                })
            })
        }

        function handlingStep_3(){

            let formValidate = wrapFormValidate[3].querySelector('.form-validate')
            let wrap_form_item_boy = formValidate.querySelector('.boy.sex-item')
            let wrap_form_item_girl = formValidate.querySelector('.girl.sex-item')
            let wrap_sub_popup = formValidate.parentElement.querySelector('.wrap-sub-popup')
            let listInputRadioOpen = formValidate.parentElement.querySelectorAll('input[name="open-sub-popup"]')

            if( userRegister.sex == '1' ) {
                wrap_form_item_boy.classList.add('active')
                wrap_form_item_girl.classList.remove('active')

                let input_job_boy = formValidate.querySelector('.txt_job_boy')
                let input_salary_boy = formValidate.querySelector('.txt_salary_boy')
    
                input_job_boy.addEventListener('focus', function(){
                    listInputRadioOpen[2].checked = "true"  // active form body girl
                    let popupJobBoy = wrap_sub_popup.querySelector('.sub-popup.popup-job-boy')
                    handlingStep3_getValue( popupJobBoy ,input_job_boy, listInputRadioOpen )
                })
    
                input_salary_boy.addEventListener('focus', function(){
                    listInputRadioOpen[3].checked = "true"  // active form attitude 
                    let popupSalaryBoy = wrap_sub_popup.querySelector('.sub-popup.popup-salary-boy')
                    handlingStep3_getValue( popupSalaryBoy ,input_salary_boy, listInputRadioOpen )
                })
            }else{
                wrap_form_item_girl.classList.add('active')
                wrap_form_item_boy.classList.remove('active')

                let input_body_girl = formValidate.querySelector('.txt_body_girl')
                let input_attitude_girl = formValidate.querySelector('.txt_attitude')
    
                input_body_girl.addEventListener('focus', function(){
                    listInputRadioOpen[0].checked = "true"  // active form body girl
                    let popupBodyGirl = wrap_sub_popup.querySelector('.sub-popup.popup-body-girl')
                    handlingStep3_getValue( popupBodyGirl ,input_body_girl, listInputRadioOpen )
                })
    
                input_attitude_girl.addEventListener('focus', function(){
                    listInputRadioOpen[1].checked = "true"  // active form attitude 
                    let popupBodyGirl = wrap_sub_popup.querySelector('.sub-popup.popup-attitude-girl')
                    handlingStep3_getValue( popupBodyGirl ,input_attitude_girl, listInputRadioOpen )
                })
            }
        }
      
        function handlingStep3_getValue (wrap, input, listInputRadio){
            let valueLabel = wrap.querySelectorAll('.wrap-option .option-item')

            valueLabel.forEach( item => {
                item.addEventListener('click', function(){
                    let valueLabel = this.querySelector('label').innerHTML
                    let valueInput = this.querySelector('input').value
                    let nameInput = this.querySelector('input').name
                    userRegister[nameInput] = valueInput
                    input.value = valueLabel // set value cho input
                    listInputRadio.forEach( item => {
                        item.checked = "" // remmove checked all input
                    })
                })
            })

            let btn_close = wrap.querySelector('.btn-back')
        ...
        function activeForm (indexForm){
            wrapFormValidate.forEach(form => {
                form.classList.remove('active')
            })
            wrapFormValidate[indexForm+1].classList.add('active')
        }

        function step_0(currentForm){ // curr = 0
            let formValidate = wrapFormValidate[currentForm].querySelector('.form-validate')
            let txt_name = formValidate.querySelector('.txt_name').value
            userRegister.fullName = txt_name
            activeForm(currentForm)
        }

        function step_1(currentForm){ // curr = 1
            let formValidate = wrapFormValidate[currentForm].querySelector('.form-validate')
            let txt_sex = formValidate.querySelectorAll('input[name="sex"]')
            txt_sex.forEach(txtItem => {
                if(txtItem.checked) {
                    userRegister.sex = txtItem.value
                }
            })
            activeForm(currentForm)
        }

        function step_2(currentForm){ // curr = 2
            let formValidate = wrapFormValidate[currentForm].querySelector('.form-validate')
            let showMessError = formValidate.querySelector('.input-error')

            let wrap_day = formValidate.querySelector('.day')
            let wrap_month = formValidate.querySelector('.month')
            let wrap_year = formValidate.querySelector('.year')

            let day = step_2_convertToNumber(wrap_day),
                month =  step_2_convertToNumber(wrap_month),
                year =  step_2_convertToNumber(wrap_year)

            if(!step2_checkDate(day, month, year)) { // error date
                showMessError.classList.add('error')
                setMessErrorInput(showMessError, 'Ngày nhập không hợp lệ!')
            }else{
                let sex = userRegister.sex
                if(sex == '1'){ // nam
                    if(step_2_age(day, month, year) < 21) {
                        setMessErrorInput(showMessError, 'Bạn phải trên 21 tuổi để sử dụng Waodate!')
                    }else{
                        if(day < 10) {
                            day = '0' + day
                        }
                        ...
                        userRegister.birthday =`${day}-${month}-${year}`
                        handlingStep_3()
                        activeForm(currentForm)
                    }
                }
                if(sex == '0'){ // nu
                    if(step_2_age(day, month, year) < 18 ) {
                        setMessErrorInput(showMessError, 'Bạn phải trên 18 tuổi để sử dụng Waodate!')
                    }else{
                        if(day < 10) {
                            day = '0' + day
                        }
        ...
        function step_3(currentForm){ // curr = 4
            let formValidate = wrapFormValidate[currentForm].querySelector('.form-validate')
            let txt_email = formValidate.querySelector('.txt_email')
            if (userRegister.sex = '1'){
                let txt_job = formValidate.querySelector('.txt_job_boy')
                let txt_salary = formValidate.querySelector('.txt_salary_boy')
                if(txt_job.value.length != 0 && txt_salary.value.length != 0 ) {
                    userRegister.email = txt_email.value
                    activeForm(currentForm)
                }else{
                   if(txt_job.value.length == 0) {
                        txt_job.classList.add('error')
                        setMessErrorInput(txt_job, 'Vui lòng điền đẩy đủ thông tin!')
                   }else{
                        txt_job.classList.remove('error')
                   }
                   if(txt_salary.value.length == 0 ) {
                        txt_salary.classList.add('error')
                        setMessErrorInput(txt_salary, 'Vui lòng điền đẩy đủ thông tin!')
                   }else{
                    txt_salary.classList.remove('error')
                   }
                }
            }
            if(userRegister.sex = '0'){
                let txt_body = formValidate.querySelector('.txt_body_girl')
                let txt_attitude = formValidate.querySelector('.txt_attitude')
                if(txt_body.value.length != 0 && txt_attitude.value.length != 0 ) {
                    userRegister.email = txt_email.value
                    activeForm(currentForm)
                }else{
                   if(txt_body.value.length == 0) {
                        txt_body.classList.add('error')
                        setMessErrorInput(txt_body, 'Vui lòng điền đẩy đủ thông tin!')
                    }else{
                        txt_body.classList.remove('error')
                    }
                   if(txt_attitude.value.length == 0 ) {
                        txt_attitude.classList.add('error')
                        setMessErrorInput(txt_attitude, 'Vui lòng điền đẩy đủ thông tin!')
                   }else{
                        txt_attitude.classList.remove('error')
                    }
                }
            }
        }

        function step_4(currentForm){ // curr = 5
            let formValidate = wrapFormValidate[currentForm].querySelector('.form-validate')
            let input_province = formValidate.querySelector('.txt_province')
            let txt_district = formValidate.querySelector('.txt_district').value
            if(step4_checkIsExistProvince(input_province.value)) {
                if(txt_district != '-1') {
                    userRegister.province = input_province.dataset.code
                    userRegister.district = txt_district
                    activeForm(currentForm)
                }else{
                    input_province.classList.add('error')
                    setMessErrorInput(input_province, 'Địa chỉ nhập không hợp lệ')
                }
            }else{
                input_province.classList.add('error')
                setMessErrorInput(input_province, 'Địa chỉ nhập không hợp lệ')
        ...
        function step_2_convertToNumber(wrapInput){
            let result = ""
            let inputItem = wrapInput.querySelectorAll('.input-item')
        ...
        function step_2_age(day, month, year) {
            let birthday = new Date(`${month}/${day}/${year}`);
            let DateNow = new Date()
            let oneDay = 24*60*60*1000;
            let age = Math.round(Math.abs((birthday.getTime() - DateNow.getTime())/(oneDay)));
            let resultYear = Math.ceil(age / 365) - 1;
            return resultYear
        }

        function step2_checkDate(day, month, year){
        ...
        function step4_checkIsExistProvince(txt_province){
            for( let index in provinces) {
                if(provinces[index].name === txt_province){
                    return true
                }
            }
            return false
        ...
        /* next form input when the current input has a value step - 3  */
        let listInputBirthday = pageLogin.querySelectorAll('.wrap-popup-item.step-3 .input-item')

        for(let i = 0; i < listInputBirthday.length -1 ; i++ ) {
            listInputBirthday[i].addEventListener('keyup', function(){
                let regex = /\d/g
                if(regex.test(this.value)){
                    listInputBirthday[i+1].focus()
                    listInputBirthday[i+1].select()
                }
            })
        }

        for(let i = listInputBirthday.length-1 ; i > 0 ; i-- ) {
            listInputBirthday[i].addEventListener('keyup', function(e){
                if (e.keyCode == 8 ) {
                    listInputBirthday[i-1].focus()
                    listInputBirthday[i-1].select()
                }
        ...
        /**
         * FUNCTION KHI KHÔNG SUBMIT
         */
        eventChooseAddressUser() // step 4

        for(let i = 0; i < listFormValidate.length; i++) {
            listFormValidate[i].addEventListener('submit', function(e) {
                e.preventDefault()
                switch (i) {
                    case 0:
                        step_0(0)
      ...
      /*
    * POPUP CREATE CONCEPT  
    */

   let btn_open_createConcept = document.querySelector('.app .btn-open-createConcept')
   
   if( btn_open_createConcept != null ) {
        let user_createConcept = {}
        let wrap_popup_createConcept = document.querySelector('.app .wrap-popup-create-concept')
        let list_btn_close_createConcept = wrap_popup_createConcept.querySelectorAll('.close-popup-createConcept')
        let list_radio_open_step = wrap_popup_createConcept.querySelectorAll('input[name="step-concept"]')
        let list_form_validate = wrap_popup_createConcept.querySelectorAll('.popup-create-concept .wrap-step form.form-validate')

        function next_popupConcept(index) {
            list_radio_open_step[index+1].checked = "true"
        }


        function submit_createConcept_step_0(current){
            let form_validate = list_form_validate[current]
            let input_sex = form_validate.querySelectorAll('input[name="sex-concept"]')
            input_sex.forEach( inputItem => {
                if(inputItem.checked) {
                    user_createConcept.sex = inputItem.value
                }
            })

            next_popupConcept(current)
        }

        function submit_createConcept_step_1(current) {
            next_popupConcept(current)
        }

        function submit_createConcept_step_2(current) {
            next_popupConcept(current)
        }

        function handling_step_1(){
            let form_validate = list_form_validate[1]
            let list_input_location = form_validate.querySelectorAll('.step2_local')
            let popup_choose_location = wrap_popup_createConcept.querySelector('.wrap-popup-concept.step-2 .sub-popup-createConcept')

            list_input_location.forEach( (inputItem, index) => {
                inputItem.addEventListener('focus', function(){
                    popup_choose_location.classList.add('active')
                    handling_step_1_subPopup(list_input_location[index])
                })
            })
            
        }

        function handling_step_1_subPopup(input_focus){
            let popup_choose_location = wrap_popup_createConcept.querySelector('.wrap-popup-concept.step-2 .sub-popup-createConcept')
            let close_popup_choose_location = popup_choose_location.querySelector('.close-sub-popup')
            let input_search = popup_choose_location.querySelector('input[name="txt_searchLocation"]')
            let list_location_item = popup_choose_location.querySelectorAll('.location-item')
            let add_new_location = popup_choose_location.querySelector('.wrap-add-newLocation')
            let popup_concept_details = popup_choose_location.querySelector('.popup-details-location')
            let btn_choose_popupDetails = popup_concept_details.querySelector('.btn_choose_location_details')

            input_search.addEventListener('keyup', function(){
                let text_input = this.value.toUpperCase()
                let txt_in_locationItem
                let count_result = 0
                for(let i = 0; i < list_location_item.length; i ++ ) {
                    txt_in_locationItem = list_location_item[i].querySelector('.name-location').innerText
                    if (txt_in_locationItem.toUpperCase().indexOf(text_input) > -1) {
                        list_location_item[i].style.display = "";
                        count_result++
                    } else {
                        list_location_item[i].style.display = "none";
                    }
                }

                if( count_result == 0 ) {
                    add_new_location.classList.add('active')
                }else{
                    add_new_location.classList.remove('active')
                }

            })

            for(let i = 0; i < list_location_item.length; i++) {
                let btn_choose = list_location_item[i].querySelector('.btn-choose-location')
                btn_choose.addEventListener('click', function(){
                    let name_location = list_location_item[i].querySelector('.name-location').innerText
                    let address_location = list_location_item[i].querySelector('.address-location').innerText
                    input_focus.value = `${name_location}, ${address_location}`
                   
                    popup_choose_location.classList.remove('active')
                })
            }

            // choose in concept details
            function chooseConcept_details(){
                
                let checkbox_open = popup_choose_location.querySelector('input[id="open_popup_details_location"]')
                let name_location = popup_concept_details.querySelector('.name-location').innerText
                let address_location = popup_concept_details.querySelector('.address-location').innerText
                input_focus.value = `${name_location}, ${address_location}`
                popup_choose_location.classList.remove('active')
                checkbox_open.checked = ''
            }

            btn_choose_popupDetails.addEventListener('click', chooseConcept_details)

            close_popup_choose_location.addEventListener('click', function () {  
                popup_choose_location.classList.remove('active')
            })
        }
        ...
        /**
         * FUNCTION XỬ LÍ TRONG CÁC STEP
         */
        handling_step_1() 
        

        // BUTTON CLOSE ALL (TOP) POPUP CREATE CONCEPT
        list_btn_close_createConcept.forEach(btn_close => {
            btn_close.addEventListener('click', function () {
                list_radio_open_step.forEach( radio_item => {
                    radio_item.checked = ""
                })
            })
        })

        for(let i = 0; i < list_form_validate.length; i ++) {
            list_form_validate[i].addEventListener('submit', function(e){
                e.preventDefault()
                switch (i) {
                    case 0:
                        submit_createConcept_step_0(0)
                        break;
                    case 1:
                        submit_createConcept_step_1(1)
                        break;
                    case 2:
                        submit_createConcept_step_2(2)
                        break;
                    default:
                        break;
                }
